  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{isEditing ? 'Blog-Post bearbeiten' : 'Neuen Blog-Post erstellen'}</DialogTitle>
          <DialogDescription>
            {isEditing ? 'Bearbeiten Sie Ihren Blog-Post.' : 'Erstellen Sie einen neuen Blog-Post für Ihre Website.'}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <Button type="submit" className="w-full bg-[#f0511e] text-white hover:bg-[#d4430f]">
            {isEditing ? 'Blog-Post aktualisieren' : 'Blog-Post veröffentlichen'}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
